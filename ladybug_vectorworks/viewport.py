...
def viewport_vh_vv(viewport, view_type):
	...
	"""
	ViewAngle = float(vs.GetRField(viewport, 'RenderWorks Camera', '__ViewAngle2'))
...
def viewport_properties(viewport, view_type=None):
	...
	LineLength = float(vs.GetRField(viewport, 'RenderWorks Camera', 'LineLength'))
	CameraZPos = float(vs.GetRField(viewport, 'RenderWorks Camera', 'CameraZPos'))
	LookToZPos = float(vs.GetRField(viewport, 'RenderWorks Camera', 'LookToZPos'))
	lpr = math.radians(vs.GetSymRot(viewport))
	(lp1x, lp1y, lp1z) = vs.GetSymLoc3D(viewport)
	(layz, layza) = vs.GetLayerElevation(vs.GetLayer(viewport))
	lp1z += layz +CameraZPos
	vx = LineLength * math.cos(lpr)
	vy = LineLength * math.sin(lpr)
	vz = LookToZPos - CameraZPos
