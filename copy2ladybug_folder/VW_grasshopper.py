...
def set_icon(node_handle,ox,oy):
	import random, string
	def randomname(n):
		randlst = [random.choice(string.ascii_letters + string.digits) for i in range(n)]
		return ''.join(randlst)
	name = "icon_"+vs.GetRField(node_handle,'MarionetteNode','NodeType')
	nname = vs.GetName(node_handle)
	if nname == "":
		nname = "lbtnode_"+ randomname(8)
		vs.SetName(node_handle, nname)
	iname = "ni_"+ vs.GetName(node_handle)
	ih = vs.GetObject(iname)
	if ih == None:
		ih =  vs.CreateDuplicateObject(vs.GetObject( name ), vs.GetParent(node_handle))
		vs.SetName(ih , iname)
	((p1x,p1y),(p2x,p2y))=vs.GetBBox(ih)
	#pw = float(vs.GetRField(node_handle,'MarionetteNode','PosWasX'))
	#ph = float(vs.GetRField(node_handle,'MarionetteNode','PosWasY'))
	(p3x,p3y)=vs.Get2DPt(node_handle)
	vs.HMove(ih,p3x-(p2x+p1x)/2+ vs.GetLScale(vs.ActLayer())/50*ox,p3y-(p2y+p1y)/2+ vs.GetLScale(vs.ActLayer())/50*oy)
